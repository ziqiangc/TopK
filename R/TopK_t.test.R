#' TopK exact test based on t test p-values (just for debugging)
#'
#' This function perform TopK exact test on a two group data set (Case-Control study).
#' The univariate p-values based on t-test.
#'
...
#'
#' @examples TopK_t.test(X,7,7)
#'
#' @export

TopK_t.test=function(X,nA,nB,
    ...
    if(vrb) cat("   Getting t values.",fill=T)

    tScan <- function(i) t.test(X[i, 1:nA], X[i, (nA+1):(nA+nB)], alternative = alternative)$p.value
    ExactWRS=t(mapply(tScan, 1:nrow(X)))
